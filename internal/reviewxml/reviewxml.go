...
func render(root, filePath string, format planformat.Format, plan string, threads []session.Thread, answers []session.SideAnswer, revisionID string, target session.Anchor, allowedStart, allowedEnd int, instruction string) string {
	var out bytes.Buffer
	out.WriteString("<")
